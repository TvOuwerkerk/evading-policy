...
## Additions made for Evading the Policy

Note: some of the same added functionality was independently added in the [original project](https://github.com/duckduckgo/tracker-radar-collector) after this version was used for research.

Additions:

1. `/helpers/cmpDetect.js`, `/helpers/utils.js`: to include [Consent-O-Matic](https://github.com/cavi-au/Consent-O-Matic) in the functionality of Tracker Radar Collector.
2. `/collectors/ScreenshotCollector.js`: for use in manual inspection of functionality to reject tracking consent dialogs.
3. Two command line options:
   - `-s, --scrape-links` to collect and save links found on visited pages
   - `-j, --input-json <path>` to use a custom scheme for inputting URLs to crawl and outputting collected data.
